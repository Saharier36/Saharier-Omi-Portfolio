        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            <div key={index} className="glass-card p-10 rounded-[32px] border-white/5 hover:border-white/10 transition-all duration-500 hover:-translate-y-2 group text-center relative overflow-hidden">
              <div className={`absolute top-0 left-0 w-1 h-full bg-gradient-to-b ${stat.color === 'text-brand-accent' ? 'from-brand-accent' : stat.color === 'text-brand-purple' ? 'from-brand-purple' : stat.color === 'text-yellow-400' ? 'from-yellow-400' : 'from-brand-blue'} to-transparent opacity-50`}></div>
